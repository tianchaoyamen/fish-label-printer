    ...
    with sync_playwright() as p:
        ...
        if clicked:
            ...
            try:
                ...
                # 在有界面模式下尝试打开目录；无头仅打印路径
                if not headless:
                    try:
                        os.startfile(str(target.parent))
                        print(f"✓ 已打开下载目录: {target.parent}")
                    except Exception:
                        try:
                            subprocess.run(["explorer", str(target.parent)])
                        except Exception:
                            print("⚠ 无法自动打开下载目录，请手动打开查看。")
                else:
                    print(f"下载目录（无头模式）：{target.parent}")
...
def adjust_excel_fit(path_or_file):
    ...
    for ws in wb.worksheets:
        try:
            ...
            # ---------- 打印设置 ----------
            try:
                ws.print_title_rows = "1:4"          # 重复 1-4 行为表头
                ws.page_setup.orientation = ws.ORIENTATION_LANDSCAPE  # 横向
                ws.page_setup.fitToWidth = 1         # 所有列装入 1 页宽
                ws.page_setup.fitToHeight = 0        # 高度不限制（0=自动）
                ws.page_setup.paperSize = ws.PAPERSIZE_A4  # 纸张大小（可选）

                # 窄边距：上下 0.5 inch，左右 0.3 inch，页眉页脚 0.2 inch
